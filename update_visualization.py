...
import plotly.express  # plotly the things
...
fig.update_xaxes(
    visible=False,
    autorange=False,
    range=[numpy.min(x_data) * 1.05, numpy.max(x_data) * 1.05],
)
...
fig.update_yaxes(
    ...
    scaleanchor="x",
    scaleratio=1,
    range=[numpy.min(y_data) * 1.05, numpy.max(y_data) * 1.05],
)
